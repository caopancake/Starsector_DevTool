...
主窗口历史命令分派机制负责把主窗口全局键盘输入解析为历史命令，并分派到当前 CSV 草稿 history 或当前 Mod 文件级 history。

## 参考

- `src/app/AppContent.vue`：在主窗口应用内容挂载时接入主窗口快捷键 composable，并把表格工具栏按钮维持在当前 CSV 草稿动作边界。
- `src/app/composables/use-main-window-shortcuts.ts`：拥有主窗口 `keydown` 监听、正式历史命令触发和默认浏览器行为拦截。
- `src/domain/workspace/main-window-commands.ts`：拥有主窗口键盘事件到历史命令的纯解析规则，包括 editable 过滤、Primary 修饰键和 Alt 排除。
- `src/domain/tables/csv-edit-history.ts`：拥有 CSV 草稿历史 entry 到 CSV Table Draft Session 回放入口的适配。
- `src/domain/tables/csv-table-draft.ts`：拥有 CSV draft operation 的 undo/redo 受控回放和 dirty 规则。
- `src/orchestrators/file-history-replay.orchestrator.ts`：拥有文件级 history 确认 UI 和 feedback。
- `src/orchestrators/file-history-session.orchestrator.ts`：拥有文件级 history replay plan、栈顶复核、changeset 回放、session refresh、跨窗口文本同步和栈提交。
- `src/orchestrators/main-history-command.orchestrator.ts`：拥有主窗口历史命令在 CSV 草稿 history 与文件级 history 之间的优先级分派。
...
- `Primary+Shift+Z` 与 `Primary+Y` 都是主窗口重做输入，分派层不得把两者拆给不同 history 系统。
- `Primary+Z` 是主窗口撤销输入，只有未按 Shift 时进入撤销分派；`Primary` 是 Ctrl 或 Meta。
- 文件级 history 回放执行归 File History Session 拥有，主窗口分派和 replay orchestrator 不直接调用 write service 或 Tauri command。
- 文件级 history 栈归文件 history store 按 `modRoot` 隔离，主窗口分派不得跨 Mod 查找可回放 entry。
- 键盘命令解析归 `main-window-commands.ts` 拥有，快捷键 composable 不直接解释 `ctrlKey`、`metaKey`、`key` 或 DOM selector。
- 键盘监听归主窗口快捷键 composable 拥有，dispatcher 不绑定 DOM 事件。
- 输入控件、textarea、select 和 contenteditable 内的键盘事件归控件自身拥有，主窗口快捷键不得拦截。
- 表格工具栏按钮只调用当前 CSV 草稿 undo/redo，不进入文件级 history fallback。
- 主窗口分派只消费 project、tables、CSV 草稿 history、CSV Table Draft Session 回放结果和文件级 history replay 的公开入口，不拥有它们的栈结构或应用算法。
- 主窗口快捷键只在主窗口应用内容挂载时注册，独立编辑器窗口和文件编辑器窗口使用自身窗口内快捷键。
- 失败反馈归被调用的分派或回放入口触发，CSV 草稿应用失败显示 CSV 编辑失败，文件级回放失败显示文件历史失败。
- 磁盘写入、changeset 应用、ProjectSession refresh 和跨窗口文本同步归文件级回放链路拥有，快捷键模块不能重写这些副作用。
...
2. 快捷键 composable 把事件交给 `mainWindowCommandFromKeyEvent()`。
3. command domain 过滤 editable target、Alt 组合键和非 Primary 组合键。
4. command domain 识别 `Primary+Z` 为 undo command。
5. 快捷键 composable 调用 `event.preventDefault()`。
6. 快捷键 composable 调用 `dispatchMainUndoCommand(feedback)`。
...
15. 用户确认后，文件级回放入口把 replay plan 交给 File History Session。
16. File History Session 重新校验栈顶和 sessionId，再回放 changeset、刷新 session、同步文本窗口并提交 history 栈。

### Primary+Y / Primary+Shift+Z 重做分派

1. `window` 收到 `keydown` 事件。
2. 快捷键 composable 把事件交给 `mainWindowCommandFromKeyEvent()`。
3. command domain 过滤 editable target、Alt 组合键和非 Primary 组合键。
4. command domain 识别 `Primary+Y` 或 `Primary+Shift+Z` 为 redo command。
5. 快捷键 composable 调用 `event.preventDefault()`。
6. 快捷键 composable 调用 `dispatchMainRedoCommand(feedback)`。
7. 主窗口分派读取 tables store、CSV 草稿 history store、project store。
8. 主窗口分派读取 `project.activeModRoot` 和 `tables.currentTab`。
...
5. CSV 草稿 history store 请求 CSV Table Draft Session 应用 CSV 草稿 entry，成功后移动 CSV 草稿栈。
...
- CSV 草稿 history 的 undo/redo 不弹文件级确认框，不触发磁盘写入，不调用 ProjectSession refresh。
...
- 主窗口历史 command 只支持 `Primary+Z`、`Primary+Y` 和 `Primary+Shift+Z`，不承载保存、删除、新建、导航或设置快捷键。
- `Alt` 参与时不得触发主窗口历史 command。
...
- 把 Primary+Z 直接接到文件级 history，会让未保存 CSV 草稿还在当前表时先回放磁盘 changeset。
- 把输入框内 Primary+Z 拦截为主窗口撤销，会破坏字段编辑器、搜索框和表格单元格编辑器的原生编辑行为。
