...
- `src/orchestrators/file-history-replay.orchestrator.ts`：拥有文件级 history 回放后的文件编辑器文本同步事件。
- `src/orchestrators/file-save.orchestrator.ts`：拥有保存事件进入主窗口后的 history 记录、session 身份校验和写入后失效刷新。
- `src/orchestrators/project-session-invalidation.orchestrator.ts`：拥有 ProjectSession 写入后刷新、manifest 更新、本地 cache 失效和跨窗口失效广播。
...
- 文件编辑器窗口身份归文件编辑器窗口入口拥有，由 modRoot 和 path 的结构化组合表达。
- 文件编辑器聚焦上下文归文件编辑器窗口入口拥有，既有窗口复用时只能通过聚焦事件覆盖上下文。
- 文件历史文本同步归文件历史回放编排拥有，文件编辑器窗口只消费命中自身 modRoot 和 path 的文本。
- 窗口尺寸归对应业务窗口入口拥有，托管窗口入口只接收并转交尺寸。
- 编辑器类窗口身份归编辑器窗口入口拥有，由 kind、modRoot 和 id 的结构化组合表达。
- 跨窗口 ProjectSession 失效归 session 失效编排拥有，窗口机制只负责广播和监听，不直接刷新业务数据。
...
3. 主窗口保存监听收到编辑器保存事件。
4. 主窗口保存编排校验事件 sessionId 和 modRoot 是否匹配当前 ProjectManifest。
5. 主窗口保存编排把 WriteResult.changes 写入文件级 history。
6. 主窗口保存编排按 WriteResult.invalidatedPaths 刷新 ProjectSession。
7. session 失效编排更新主窗口 ProjectManifest。
8. session 失效编排清理主窗口本地 query cache 和 resource cache。
9. session 失效编排广播 ProjectSession 失效事件。
10. 编辑器窗口收到编辑器保存事件后，按自身目标和当前 bundle 判断是否应用保存后的 spec。
...
2. 文件历史回放编排调用写入服务回放 changeset。
3. 文件历史回放编排遍历回放涉及的非目录变更。
4. 文本变更存在可回放文本时发送文件编辑器文本应用事件。
5. 文件编辑器窗口收到文本应用事件。
6. 文件编辑器 ViewModel 按 modRoot 和 path 匹配当前窗口目标。
7. 匹配成功时文件编辑器 ViewModel 替换文本快照并清空草稿 undo/redo。
8. 文件历史回放编排按 WriteResult 刷新受影响的已加载 ProjectSession。

### ProjectSession 失效事件

1. 主窗口保存编排取得 WriteResult.invalidatedPaths。
2. session 失效编排按当前 ProjectManifest 过滤项目作用域路径。
3. session 失效编排调用 session 服务刷新 ProjectSession。
4. session 失效编排更新主窗口 ProjectManifest。
5. session 失效编排清理主窗口本地 query cache 和 resource cache。
6. session 失效编排广播 ProjectSession 失效事件。
...
10. 编辑器窗口 ViewModel 按 query cache 失效类型重查主实体或刷新派生数据。
...
4. 主窗口保存监听收到贴图上传保存事件。
5. 主窗口保存编排校验事件 sessionId 和 modRoot 是否匹配当前 ProjectManifest。
6. 主窗口保存编排把 WriteResult.changes 写入文件级 history。
7. 主窗口保存编排按 WriteResult.invalidatedPaths 刷新 ProjectSession。
8. session 失效编排更新主窗口 ProjectManifest。
9. session 失效编排清理主窗口本地 query cache 和 resource cache。
10. session 失效编排广播 ProjectSession 失效事件。

## 规范

- URL 参数序列化只能省略 null 和 undefined，空字符串必须作为显式输入传递。
- ProjectSession 失效事件必须携带更新后的 manifest 和 invalidatedPaths。
...
- 文件编辑器文本应用事件必须携带 modRoot、path 和 text。
