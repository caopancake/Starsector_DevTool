...
- `theme/accent/editMode` 使用正式枚举；字段入口消费 editMode。主题只由 store、root token 和 provider 驱动；Naive UI 的 common 字体、图标、边界、表面和主强调色直接引用同一组 root token，禁止读取 DOM 计算色值或保留独立的日/夜回退色。
