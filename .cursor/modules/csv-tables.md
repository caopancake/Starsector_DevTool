...
- `src/domain/tables/associated-specs.ts`：拥有公开 CSV 表到关联 spec 文本路径 UI 映射和专用编辑器窗口 kind 的映射。
...
- `src/domain/tables/csv-table-draft.ts`：拥有 CSV Table Draft Session、dirty 规则、window 合并、行增删、history operation 回放、保存后 baseline 和外部更新标记。
...
- `src/orchestrators/table-save.orchestrator.ts`：拥有保存目标捕获、patch 构造、关联 spec 确认后写入、rowKey 映射和 history/cache 触发。
- `src/services/csv-table.service.ts`：拥有 CSV table window、source options、row preview 和 table patch 保存 service 入口。
- `src/stores/tables.store.ts`：拥有按 modRoot 隔离的 CSV 表格状态容器、选择、编辑入口和当前表状态，并把草稿规则委托给 CSV Table Draft Session。
- `src-tauri/src/services/project/query/csv_window.rs`：拥有 Rust CSV window query、row preview、搜索和势力筛选。
- `src-tauri/src/services/project/table_definitions.rs`：拥有 Rust CSV table 路径、entity id 字段、entity summary 来源、行资源和 source option 资源抽取定义。
- `src-tauri/src/services/project/write/csv_patch.rs`：拥有 Rust CSV patch 应用、关联 spec 动作到 changeset 的转换和 rowKey 映射。
...
- Grid 渲染层只承载展示、虚拟化和编辑控件，不拥有写盘、history 或关联 spec 语义。
- ModTableState 归 tables store 拥有，必须按 modRoot 隔离。
- CSV Table Draft Session 是 `tables/originalTables/dirty/pendingExternalTableUpdates` 的唯一规则 owner；store 只能定位状态并调用 domain 入口。
- RowData 中的 rowKey 是运行态身份，不写入磁盘。
- TableKey 归共享类型和 Rust CsvTableKey 拥有，前端 store 不得维护独立表注册表。
- Rust CSV table definition 是表路径、实体计数 ID 字段、source option 资源和 row preview 资源的唯一后端定义入口。
- Skill entity 以 `skill_data.csv` 注册行为列表和详情边界，`.skill` 文件只是已注册 skill 的关联 spec 数据。
...
- 关联 spec 详情页文本入口路径和专用窗口映射归 associated spec domain 拥有；关联 spec 写盘路径、扩展名、ID 字段和默认内容归后端实体定义拥有。
...
4. CSV ViewModel 因用户切换表、搜索或筛选请求 CSV Table Draft Session 丢弃当前表窗口草稿。
...
14. tables store 调用 CSV Table Draft Session 应用 window。
15. 目标表无 dirty 且无编辑中单元格时，CSV Table Draft Session 合并 window rows 和 original rows。
16. 目标表存在 dirty 或编辑中单元格时，CSV Table Draft Session 只标记文件级外部更新，不写入 rows、original rows、selection 或 editing。
...
6. tables store 调用 CSV Table Draft Session 提交单元格值。
7. CSV Table Draft Session 按当前表和 rowKey 查找已加载真实行。
8. CSV Table Draft Session 写入单元格值，对比 original row 生成或清理 dirty cells。
9. CSV Table Draft Session 清除 editing 状态，并返回 `cell-value-set` operation。
10. tables store 把返回的 operation 推入 CSV 草稿历史栈。
...
3. 新增行时 CSV Table Draft Session 按当前表 header 创建空行。
4. 新增行时 CSV Table Draft Session 分配 `${table}:new:*` 临时 rowKey。
5. 新增行时 CSV Table Draft Session 标记整行 dirty、选中新行，并返回 `row-created` operation。
6. 删除行时 CSV Table Draft Session 按 selectedRowKey 查找已加载真实行。
7. 删除行时 CSV Table Draft Session 从当前表 rows 中移除该行。
8. 删除原始已存在行时 CSV Table Draft Session 写入 delete dirty row。
9. 删除未保存新增行时 CSV Table Draft Session 清除该 rowKey 的 dirty。
10. tables store 把返回的 `row-created` 或 `row-deleted` operation 推入 CSV 草稿历史栈。
...
4. table save orchestrator 捕获 manifest、modRoot、ModTableState、table 和关联 spec 候选。
5. 需要关联 spec 确认时 workspace shell 打开确认对话。
6. 用户确认后调用 save captured table changes。
7. table save orchestrator 校验捕获 state 和 session 仍是当前目标。
8. table save orchestrator 过滤当前表关联 spec 动作。
...
13. Rust write 渲染 CSV 文本，并按后端实体定义把关联 spec 动作构造成 changeset。
14. Rust write 应用 changeset 并返回 WriteResult 和 rowKey keyMap。
15. table save orchestrator 把写入结果交给 File History Session。
16. File History Session 记录文件级 history，并按 WriteResult.invalidation.paths 刷新 ProjectSession 和前端 cache。
17. File History Session 完整成功后，table save orchestrator 请求 CSV Table Draft Session 应用 rowKey 映射。
18. tables store 请求 CSV Table Draft Session 标记捕获表已保存并清理 dirty。

### query cache 失效响应

1. ProjectSession refresh 清理 query cache。
2. query cache 发布失效事件。
3. CSV ViewModel 校验事件 sessionId 是否匹配当前 active session。
4. 当前表 csv-table-window 失效且当前表无 dirty 时，CSV ViewModel 重载当前 table window。
5. 当前表 csv-table-window 失效、搜索、筛选、分页或其它 window reload 命中 dirty 表时，tables store 只标记当前 CSV 文件存在外部更新，不覆盖 rows、original rows、selection、editing 或 dirty。
6. 用户点击载入外部版本时，CSV ViewModel 明确丢弃当前 dirty、清空文件级外部更新标记并重载当前 table window。
7. query cache 按后端 `csv-source-options` scope 的 `source` 或 `table` 命中可见 source 后，CSV ViewModel 重载 source options。
8. resource cache 事件命中 loaded source option 持有的 `ResourceRef` 时 CSV ViewModel 重载 source options。
9. 当前 session 消失时 CSV ViewModel 清空 window key、source option index 和列宽派生状态。
...
- CSV dirty 规则只能在 CSV Table Draft Session 中实现；store、history store 和组件不得直接解释 dirty。
- CSV 文件级外部更新标记只表达当前 CSV 文件被外部刷新命中，不做行级或单元格级冲突分析。
- CSV dirty 与文件级外部更新同时存在时，保存允许用当前 dirty 覆盖外部版本；保存成功后清空 dirty 和外部更新标记。
- CSV dirty 表不得应用任何服务端 window 结果；载入外部版本是唯一允许丢弃 dirty 并重载窗口的入口。
- CSV query 的可空过滤字段必须显式提交 null。
- CSV source option 查询必须校验 source 声明列存在于 CSV header。
- CSV source 选项只有 id 列可以携带实体资源引用，资源抽取规则必须来自 Rust CSV table definition。
- skills 表的 `icon` 列是技能 entity 图标资源来源；缺失 `.skill` 文件不得让已注册技能从 entity list 消失。
...
- 顶部撤销、重做和保存只作用于当前表。
- 右侧详情动作必须携带发起时的 modRoot、sessionId、业务目标和 action type；专用编辑器动作必须携带 starsectorRoot、编辑器 kind 和业务 id，文件编辑器动作必须携带关联 spec 路径。
- 右侧详情动作必须先显示专用编辑器和预览入口，再把关联 spec 文本文件编辑器入口追加到操作栏最下方。
- 右侧详情文件编辑器动作只传文件目标，不得把文件名作为上下文消息重复传给文件编辑器。
...
- 注释行不得作为引用、source、关联 spec 或编辑器入口的合法目标。
...
- 把详情页关联 spec 文本入口当作前端写盘权威，会恢复 createText、ID 字段或默认内容生成的旧错误边界。
