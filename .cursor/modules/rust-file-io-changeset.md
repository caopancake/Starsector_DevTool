...
Rust 文件 IO、路径校验与目录 changeset 模块负责把后端文件读写、路径归属、文件级变更记录、目录快照和失败回滚收口为统一磁盘写入模型。
...
- `src-tauri/src/io/paths.rs`：拥有绝对路径、相对路径、parent-dir 拒绝和路径身份规范化工具。
...
- `FileChangeSetBuilder` 拥有从根目录加相对路径构建文件或目录 change 的能力，业务 service 只提供已校验的保存意图和内容。
- `FileSnapshot` 只归目录 change 使用，`relPath` 必须是目录内部普通相对文件路径。
- `ProjectSession` 与 `modRoot` 匹配校验归 command 层进入写盘 service 前执行，service 层仍拥有每个 path 对 `modRoot` 的归属校验。
- `WriteResult.invalidatedPaths` 归 changeset 统一规则生成，消费方只能用它触发 session/cache 失效。
- `io/file_changes` 拥有 apply、rollback、目录恢复和二进制恢复，不能依赖 command、service、store 或前端历史状态。
- `io/paths` 拥有路径组件安全规则和路径 key 规范化，业务 service 不得自行维护另一套路由、归属或 parent-dir 判断。
...
6. file changes service 校验 `path` 是无 parent-dir 的绝对路径且归属 `modRoot`。
7. text IO 读取字节并拒绝 UTF-8 BOM。
...
10. 前端保存编排记录 file history，并按 `invalidatedPaths` 刷新 ProjectSession。
...
6. file changes service 以 `modRoot` 创建 `FileChangeSetBuilder`。
7. builder 逐个校验 `AssociatedFileChange.relPath` 是非空普通相对文件路径。
8. builder 读取每个目标当前状态，生成 file change。
9. builder apply 按 redo 写入文本、写入二进制或删除文件。
10. apply 成功后 service 返回包含 changes 和统一 invalidated paths 的 `WriteResult`。

### 业务保存消费 changeset

1. 业务 service 按自身模型完成 ID、字段、索引、目标和内容校验。
2. 业务 service 以自身声明拥有的保存根目录创建 `FileChangeSetBuilder` 或构造单个 text change。
3. 业务 service 向 builder 添加文本文件、二进制文件、目录删除或目录复制产生的目标 change。
4. builder 在添加相对路径 change 时统一拒绝空路径、`.`、`..`、绝对路径和 Windows prefix。
5. builder 在构建 change 时捕获 before 状态，文本可读时记录 text，文本不可读时记录 base64。
6. builder apply 按 redo 执行 changeset。
7. 业务 service 使用 `invalidated_paths_for_changes()` 生成失效路径。
8. 业务 service 返回包含 changes、invalidated paths、业务刷新实体、keyMap 或 warnings 的 `WriteResult`。
...
8. file changes service 校验每个 `FileChangeRecord.path` 是无 parent-dir 的绝对路径且归属 `modRoot`。
9. file changes service 校验每个目录快照 `FileSnapshot.relPath` 是目录内普通相对文件路径。
...
14. 任一 change 应用失败时，apply 按已记录的当前状态逆序回滚已应用 change。
15. apply 成功后 service 返回 `WriteResult`。
16. 前端通知打开的文件编辑窗口同步文本 change，并按 `invalidatedPaths` 刷新已加载 session。

## 规范

- `AssociatedFileChange.afterText` 和 `afterDataBase64` 必须显式可空；两者都为空表示删除文件。
- `AssociatedFileChange.previousRelPath` 只表达业务保存链路中的旧相对目标，文件 IO service 不解释它的重命名语义。
- `FileChangeKind::Directory` 的 before/after 文件内容只能存放在 `beforeFiles` 和 `afterFiles`，不能塞进单文件 text/base64 字段。
- `FileChangeKind::File` 的 `path` 必须是绝对路径；builder 从相对路径构造时必须先归属到 `modRoot`。
...
- `WriteResult.invalidatedPaths` 对 file change 包含该文件绝对路径；对 directory change 同时包含目录路径和快照内每个文件的绝对路径。
...
- `modRoot` 归属判断必须拒绝绝对路径缺失、parent-dir 组件、root parent-dir 和不在 root 内的路径。
