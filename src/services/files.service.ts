...
export function loadEditableFileData(sessionId: string, modRoot: string, path: string): Promise<EditableFileData> {
  return loadEditableFile(sessionId, modRoot, path);
}

export function writeEditableFileText(sessionId: string, modRoot: string, path: string, text: string): Promise<WriteResult> {
