...
import { saveWeapon, uploadSprite } from '../../../shared/api/tauri';
...
const offsets = computed<number[]>(() =>
  Array.isArray(localWeapon.value[offsetsKey.value])
    ? (localWeapon.value[offsetsKey.value] as number[])
    : ((localWeapon.value[offsetsKey.value] = []) as number[]),
);
const angles = computed<number[]>(() =>
  Array.isArray(localWeapon.value[anglesKey.value])
    ? (localWeapon.value[anglesKey.value] as number[])
    : ((localWeapon.value[anglesKey.value] = []) as number[]),
);
...
function objectField(key: string): RowData {
  return localWeapon.value[key] && typeof localWeapon.value[key] === 'object' && !Array.isArray(localWeapon.value[key])
    ? (localWeapon.value[key] as RowData)
    : {};
}
function pushUndo() {
  undo.push(JSON.stringify(localWeapon.value));
  if (undo.length > 250) undo.shift();
  redo.length = 0;
}
function doUndo() {
  if (!undo.length) return;
  redo.push(JSON.stringify(localWeapon.value));
  localWeapon.value = JSON.parse(undo.pop()!);
  selected.value = -1;
  draw();
}
function doRedo() {
  ...
  undo.push(JSON.stringify(localWeapon.value));
  localWeapon.value = JSON.parse(redo.pop()!);
  selected.value = -1;
  draw();
}
...
function toCanvas(x: number, y: number) {
  const cc = center();
  return { x: cc.x + y * scale.value, y: cc.y - x * scale.value };
}
function toWeapon(px: number, py: number) {
  const cc = center();
  return { x: -(py - cc.y) / scale.value, y: (px - cc.x) / scale.value };
}
...
function drawGrid(ctx: CanvasRenderingContext2D) {
  ...
  ctx.lineWidth = 0.5;
  for (let x = cc.x % step; x < c.width; x += step) {
    ctx.beginPath();
    ctx.moveTo(x, 0);
    ctx.lineTo(x, c.height);
    ctx.stroke();
  }
  for (let y = cc.y % step; y < c.height; y += step) {
    ctx.beginPath();
    ctx.moveTo(0, y);
...
function onMove(e: MouseEvent) {
  ...
  offsets.value[selected.value * 2] = +coord.x.toFixed(1);
  offsets.value[selected.value * 2 + 1] = +coord.y.toFixed(1);
  draw();
}
function onUp() {
...
function onWheel(e: WheelEvent) {
  scale.value = Math.max(0.1, Math.min(20, scale.value * (e.deltaY < 0 ? 1.1 : 0.9)));
  draw();
}
...
async function uploadCurrentSprite(event: Event) {
  ...
  let result = await uploadSprite(props.modRoot, file.name, b64, 'weapons', false);
  if (result.exists) {
    dialog.warning({
      title: '覆盖贴图？',
      ...
      onPositiveClick: async () => {
        result = await uploadSprite(props.modRoot, file.name, b64, 'weapons', true);
...
async function save() {
  await saveWeapon(props.modRoot, props.weaponId, localWeapon.value);
...
const ObjectEditor = defineComponent({
  ...
  setup(p, { emit }) {
    ...
    watch(
      () => p.modelValue,
      (v) => (text.value = JSON.stringify(v || {}, null, 2)),
    );
    const apply = () => {
      try {
