...
import { openShipEditorWindow, openWeaponEditorWindow, openWeaponPreviewWindow, type EditorSpecSavedEvent } from '../editors/editor-window';
import { useFileHistoryStore } from '../file-history/file-history-store';
import { useMainWindowShortcuts } from '../undo-redo/composables/use-main-window-shortcuts';
import { useProjectStore } from '../project/project-store';
import { pickDirectory } from '../project/project-service';
import { getAssociatedFileCandidates, saveActiveTableChanges } from '../tables/table-save-orchestrator';
import { useTablesEditHistoryStore } from '../tables/tables-edit-history-store';
import { useTablesStore } from '../tables/tables-store';
import type { AssociatedFileCandidate } from '../tables/associated-file-candidates';
import type { TableDetailAction } from '../tables/table-detail-actions';
import { listenWindowSaveEvents } from '../windowing/window-save-events';
import { openFileEditorWindow, type FileEditorRequest } from './file-editor-window';
import { loadModFromOverview, openDetectedDirectory } from './open-directory-service';
import { restorePersistedWorkspace, watchWorkspacePersistence, type WorkspacePersistenceWatcher } from './workspace-persistence';
...
export function useWorkspaceShellActions(message: MessageApiInjection, dialog: DialogApiInjection) {
  ...
  async function saveChanges() {
    try {
      ...
      const candidates = getAssociatedFileCandidates(project.activeModData);
