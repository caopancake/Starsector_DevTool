import { loadCsvTable, saveCsvWithHistory, type AssociatedFileChange } from '../../shared/api/tables-api';
...
export async function saveTableRows(
  ...
  try {
    return await saveCsvWithHistory(modRoot, table, header, rows, associatedFiles);
  } catch (error) {
