import { WINDOW_EVENTS, type ProjectSessionInvalidatedEvent } from '@/windows/window.events';
import { emitWindowEvent, listenWindowEvent, type WindowEventHandler } from '@/windows/tauri.events';
import { recordWindowEventHandlerError } from '@/orchestrators/window-event-errors.orchestrator';
import { useProjectStore } from '@/stores/project.store';
import { invalidateProject } from '@/services/session.service';
import { invalidateQueryCacheByPaths } from '@/services/query-cache.service';
import { invalidateResourceCacheByPaths } from '@/services/resource-cache.service';
import { isAbsoluteFsPath, pathBelongsToRoot, pathIsProjectScopedChangedPath } from '@/shared/lib/paths';
import type { ProjectManifest, WriteResult } from '@/shared/types';

export function emitProjectSessionInvalidated(event: ProjectSessionInvalidatedEvent) {
  return emitWindowEvent(WINDOW_EVENTS.projectSessionInvalidated, event);
}

export function listenProjectSessionInvalidated(handler: WindowEventHandler<ProjectSessionInvalidatedEvent>) {
  return listenWindowEvent<ProjectSessionInvalidatedEvent>(WINDOW_EVENTS.projectSessionInvalidated, handler, recordWindowEventHandlerError);
}

export function applyProjectSessionInvalidationToLocalCaches(event: ProjectSessionInvalidatedEvent) {
  const projectRoot = event.manifest.modRoot;
  invalidateResourceCacheByPaths(event.manifest.sessionId, projectRoot, event.invalidatedPaths);
  invalidateQueryCacheByPaths(event.manifest, event.invalidatedPaths);
}

export async function invalidateProjectSessionForWriteResult(modRoot: string, result: WriteResult, expectedSessionId?: string | null) {
  const project = useProjectStore();
  const manifest = project.getManifest(modRoot);
  if (!manifest) return null;
  if (expectedSessionId && manifest.sessionId !== expectedSessionId) return null;
  const scopedPaths = result.invalidatedPaths.filter((path) => pathIsProjectScopedChangedPath(path, manifest.modRoot));
  if (scopedPaths.length === 0) return null;
  return invalidateProjectSessionForPaths(project, manifest, scopedPaths);
}

export async function invalidateLoadedProjectSessionsForWriteResult(result: WriteResult, relativePathModRoot: string | null) {
  const project = useProjectStore();
  const events: ProjectSessionInvalidatedEvent[] = [];
  await Promise.all(
    [...project.manifests.values()].map(async (manifest) => {
      const scopedPaths = result.invalidatedPaths.filter((path) =>
        isAbsoluteFsPath(path) ? pathBelongsToRoot(path, manifest.modRoot) : manifest.modRoot === relativePathModRoot,
      );
      if (scopedPaths.length === 0) return;
      const event = await invalidateProjectSessionForPaths(project, manifest, scopedPaths);
      events.push(event);
    }),
  );
  return events;
}

async function invalidateProjectSessionForPaths(
  project: ReturnType<typeof useProjectStore>,
  manifest: ProjectManifest,
  invalidatedPaths: string[],
): Promise<ProjectSessionInvalidatedEvent> {
  const updatedManifest = await invalidateProject(manifest.sessionId, invalidatedPaths);
  project.updateManifest(manifest.modRoot, updatedManifest);
  const event = { manifest: updatedManifest, invalidatedPaths };
  applyProjectSessionInvalidationToLocalCaches(event);
  await emitProjectSessionInvalidated(event);
  return event;
}
