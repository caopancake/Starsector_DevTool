...
- workspace store 拥有全局页、已加载 Mod、按 `modRoot` 保存的 Mod 页面上下文、概览、打开失败状态、列宽和持久化投影；persistence orchestrator 自动保存/启动恢复；navigation/lifecycle orchestrator 同步跨 store、移除/关闭；游戏概览成功打开 Mod 后进入该 Mod 的概览页。
- 移除 Mod 的 5-store 清理序列（workspace/tables/fileHistory/csvEditHistory/project）唯一归属 `workspace-lifecycle.orchestrator.removeModRuntimeState`；缓存失效、Rust session 关闭与视图回退由调用方在用例之外组合，严禁第二份内联清理序列。
...
- 游戏概览、ProjectSession 与 Mod 打开失败状态必须分离并按 `modRoot` 隔离。同一 Mod 只允许保留最新一条运行时失败；成功打开对应 Mod、成功刷新或关闭工作区必须清理适用失败。恢复重新打开 session，完成后显示工作区总览；Mod 页面上下文和打开失败状态只存于本次运行，严禁作为持久化或 session 权威。
