...
- 前端、Rust 与文档文件的收集范围由共享收集器统一决定；架构、编码与标识符长度三个检查入口共用同一收集器，范围差异只允许以扩展名与包含谓词表达，规则与入口脚本严禁自建目录遍历。
- 规则判定文件 surface 只允许三种形态：共享分类器的路径角色、归一化相对路径的成员判定、`singleFileByRel()` 单文件锚定。
