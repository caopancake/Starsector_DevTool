...
- `src/app/components/config/ConfigFactionList.vue` 管理势力列表、新建和删除。
- `src/app/components/config/ConfigFactionEditor.vue` 编辑和删除 `.faction`。
...
8. Rust 以一个 changeset 写盘并返回 `IndexedConfigEntityResult`。
9. 前端记录一条文件级 history。
10. `ConfigMissionView.vue` 刷新列表，`ConfigMissionEditor.vue` 只基于 result 刷新 schema 状态和选中 ID。
