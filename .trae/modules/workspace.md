...
- `App.vue` 不直接横向编排多个 feature；主窗口用户动作通过 `workspace-shell-actions.ts` 进入对应业务模块。
...
4. Rust workspace service 读取工具私有 workspace 文件。
5. 前端按持久化的 `starsectorRoot` 重新扫描游戏概览。
6. 前端逐个调用 `restoreWorkspaceMod()`。
7. `restoreWorkspaceMod()` 调用项目完整加载。
8. 加载成功的 Mod 写入 project store、tables store 和 workspace store。
9. workspace store 恢复活动 Mod。
10. workspace store 导航到 `overview`。
11. core fields 加载器刷新全局 core 字段。
...
5. Rust workspace service 写入工具私有 workspace 文件。
