...
- `定义` 说明模块是什么、解决什么问题。
- `边界` 只写稳定的层级职责、状态归属和模块间拥有关系；不得列举会随普通功能接入持续增长的业务项。
- `规范` 只记录长期有效的模块规则、禁止边界和容易破坏架构的约束。
- `链路` 只写真实调用顺序，不能混入规范、解释或规划。
- 模块文档不得记录阶段任务完成情况。
- 模块文档不得为普通新增项追加清单式实现细节。
- 会持续增长的注册事实以代码注册表、schema 或类型定义为准；模块文档只能说明这些事实由哪个注册机制承载。
- 只有新增项改变模块边界、保存链路、状态归属、UI 模式或跨层规则时，才允许更新模块文档。
