...
- [x] 修复武器、联队、船插、工业在右侧详情面板中的预览问题，确保展示内容和资源状态与当前记录一致。
- [x] 明确武器、联队、船插、工业在无贴图、缺资源、路径失效或无专用预览时的占位规则和提示文案。
- [x] 回查右侧详情面板的缩略图、占位图和摘要信息在切换表、切换记录、切换 Mod 时是否会串状态或残留旧数据。
- [x] 确保所有涉及画布或像素资源的渲染都采用邻近采样，不允许在任何情况下退回模糊缩放。
- [x] 回查舰船编辑器、武器编辑器和发射预览，确认高倍缩放、缩略图和画布重绘时仍保持硬边缘像素效果。
- [x] 补最小手动验收清单，覆盖有贴图、无贴图、缺失贴图和高倍缩放四类场景。

## Phase 5: 重做槽位画布渲染

- [x] 收集并确认舰船武器槽、引擎槽、边界点、选中态和 hover 态的视觉规则。
- [x] 重点关注颜色、形状、线条粗细的完全一致，尤其是线条不允许过细。
- [x] 重做舰船编辑器里武器槽、引擎槽、边界点、中心点、护盾点等画布渲染样式。
- [x] 重做武器编辑器里炮口/barrel offset、角度、选中态和拖动态的画布渲染样式。
- [x] 保持当前坐标、拖拽、缩放、0.5 吸附和保存逻辑不变，只调整画布绘制视觉。
- [x] 验证高倍缩放下仍保持硬边缘像素风格，不出现模糊。
- [x] 为画布渲染规则补充文档或代码注释，避免后续偏离当前画布视觉基准。
