...
- [x] 支持统一打开目录：可选择 Starsector 游戏根目录或任意 Mod 根目录。
- [x] 自动识别游戏目录、游戏内 Mod 和外部 Mod；外部 Mod 的游戏目录 fallback 最后使用设置页路径。
- [x] 打开游戏目录时只做轻量概览扫描，列出 `mods/` 下可识别 Mod，不立即全量读取全部内容。
- [x] 概览页支持对单个 Mod 执行“完整读取”，完整读取后进入现有单 Mod 编辑链路。
- [x] 建立游戏概览状态，区分游戏目录、轻量 Mod 列表、已完整读取 Mod 和当前可编辑目标。
- [x] 明确原版数据默认只读，避免误写 `starsector-core`。
- [x] 为武器、弹体、舰船等跨 Mod 引用提供显式游戏目录 fallback 基础。
- [x] 持久化最近打开的游戏目录、轻量 Mod 概览、已完整读取 Mod 和用户视图设置。
- [x] 为游戏目录扫描、原版 fallback、多 Mod 冲突和切换编辑目标补最小测试或手动验收流程。
- [x] 文件读取/解析失败时错误提示可打开独立修复窗口，显示路径、错误消息和红色目标行高亮；保存只写回文件，不自动重新加载。
