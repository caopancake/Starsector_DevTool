...
- [x] 统一产品层术语，明确使用 `弹体`、`发射预览`、`Mod`、`总览`、`设置` 等名称。
- [x] 统一代码层命名，包括组件名、store 字段、事件名、type 名和文档标题，移除旧称和混用称呼。
- [x] 扫描 `.trae/`、`README.md`、`AGENTS.md`、前端类型和组件文件名，消除同义混用。
- [x] 明确哪些地方保留技术文件后缀，例如 `.ship/.wpn/.proj`，哪些地方使用产品层名称。
- [x] 以 `.trae/terminology.md` 为唯一术语基准修正文档、组件命名和对外文案，不再自行发明近义词。
- [x] 为后续阶段建立统一命名约束，避免再出现 UI 文案、文档标题和代码名各说各话。
