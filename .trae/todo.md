...
- [ ] 将 `src-tauri/src/services/project/mod.rs` 按正式职责重排为 session、query、write、cache 和 model 子模块。
- [ ] `session` 只负责打开、关闭、session 生命周期、session cache 和 root cache 生命周期，不承载 CSV、entity、resource 业务逻辑。
- [ ] `query` 只负责读取，按 CSV window、entity、source options、resource data URL、hull reference 分模块组织。
- [ ] `write` 只负责写入事务，按 CSV patch、config entity、文件保存、贴图上传和 history invalidation 分模块组织。
- [ ] `cache` 明确区分 session cache 与 core cache；session 按 sessionId 失效，core 按 starsectorRoot 失效。
- [ ] `model` 放 ProjectSession 内部模型、QueryKey、WriteResult、ResourceRef 辅助模型和 rowKey / patch 规则。
- [ ] Rust command 层继续只调用 service；拆分不得恢复 command 直接访问 IO、parser 或业务 helper。

### Phase 2.2: 后端读写模型统一

- [ ] 所有 query 入口统一为 `sessionId + query kind + parameters` 的模型，不允许各模块自定义不可复用 query 形态。
- [ ] 所有写入入口统一返回 changes、invalidatedPaths、必要刷新数据和 keyMap；写入结果必须能直接驱动前端 cache invalidation。
- [ ] CSV 保存、config entity 保存、文件编辑器保存、贴图上传和 history replay 必须进入同一套 invalidation 输出模型。
- [ ] source query、hull reference query 和 resource query 的 `ResourceRef` 生成规则必须统一，禁止前后端重复拼接资源路径。
- [ ] core cache 继续按类型懒加载，不允许 source query 或 resource query 预读整套 core 数据。

### Phase 2.3: 前端 API 与 Service 重排

- [ ] `src/shared/api` 按正式边界重命名和重排为 session、query、write、workspace / app config 等 wire adapter，不再按旧功能碎片命名。
- [ ] `src/services` 按业务能力收束为 session service、query service、resource cache service、config entity service、CSV table service 和 write service。
- [ ] `config.service.ts` 拆出保存、查询、hull reference 和 schema entity 相关职责；组件不得通过一个杂糅 service 横跨读写边界。
- [ ] `table.service.ts` 只保留 CSV query、source options、grid model 输入和当前表保存能力，不承载配置 entity 或编辑器候选逻辑。
- [ ] 编辑器候选项、schema 下拉、CSV 增强控件和配置新建弹窗必须复用同一套 source query / resource cache 服务。

### Phase 2.4: 前端 ViewModel 收束

- [ ] 为复杂页面建立 `*.view-model.ts`，组件只接收状态、动作和渲染数据，不直接拼 query、cache、ResourceRef 或保存 payload。
- [ ] Config Mission、Faction、Variant、Skin 优先接入 ViewModel；列表、详情、创建、删除、保存和缩略图加载由 ViewModel 统一编排。
- [ ] CSV 表格 ViewModel 统一管理当前表 window query、search、faction filter、row selection、dirty patch、草稿 history 和保存动作。
- [ ] 编辑器窗口 ViewModel 统一管理 entity query、候选 source query、资源批量加载、保存和窗口事件。
- [ ] ViewModel 不直接调用 shared/api；只能调用 service 或 orchestrator。

### Phase 2.5: 前端 Query / Cache 标准模型

- [ ] 建立统一前端 query cache 模型，key 固定为 `sessionId + query kind + normalized parameters`。
- [ ] CSV source options、hull references、resource data URLs、entity list、entity detail 和 CSV grid model 全部接入标准 cache / invalidation。
- [ ] 所有资源 data URL 继续只接受批量 `ResourceRef[]`，组件不能直接发资源 query，也不能逐项补图。
- [ ] session invalidation、changed paths、关闭 session、切换 root 必须统一清理相关 query cache 和 resource cache。
- [ ] 性能日志接入标准 query 模型，记录 query kind、命中/未命中、耗时和结果规模。

### Phase 2.6: 旧 API、旧模型和命名清理

- [ ] 删除或重命名所有仍带旧完整读取、旧 AppData、旧 config-api、旧 mission load、旧 project tables service 语义的文件、函数和类型。
- [ ] 检查 `shared/api`、`services`、`orchestrators`、`stores`、`windows` 的命名是否表达正式职责；不清晰的必须重命名。
- [ ] 清理旧兼容函数、空占位实现、超大窗口查询、重复缓存和组件内临时 Promise 编排。
- [ ] Rust 和前端测试按新模块边界重排；不保留围绕旧入口的 test-only 链路。

### Phase 2.7: 静态检查体系重写

- [ ] 删除不再适配 Phase 2 新结构的静态检查脚本；剩余检查必须重新审计职责和边界。
- [ ] 以 Phase 2 的正式目录、模型和调用链为准重写前端架构检查，禁止旧 API、旧 AppData、组件直连 query、组件直连 resource、组件内临时缓存和绕过 ViewModel 的复杂页面编排。
- [ ] 以 Phase 2 的 Rust session/query/write/cache/model 分层为准重写后端架构检查，禁止 command 直接业务、query 写盘、write 读完整项目、cache 预读整套 core 和跨层调用。
- [ ] 静态检查必须以目录角色、导入方向、类型形状和正式入口为依据；不允许业务白名单、临时例外、兼容放行或为了减少误伤而放宽边界。
- [ ] 静态检查自检也必须按新规则重写，继续禁止白名单/黑名单式漂移，并允许必要的单文件语义检查。
- [ ] 新静态检查必须覆盖 shared/api、services、orchestrators、view-model、stores、windows、Rust commands/services/domain/io/parsers/models 的长期边界。

### Phase 2.8: 模块文档体系重写

- [ ] 按 Phase 2 新架构重写 `.trae/module-map.md`，模块索引必须反映 session/query/write/cache/view-model 的正式结构。
- [ ] 重写 ProjectSession、CSV tables、config、resource/core fallback、file history、workspace、editor、schema 等受影响模块文档。
- [ ] 模块文档只记录长期定义、边界、规范和正式链路；不得记录迁移过程、临时理由、阶段过程或实现细枝末节。
- [ ] 删除仍围绕旧完整读取、旧 AppData、旧 project tables service、旧 mission load、旧 config-api 语义的文档表述。
- [ ] overview / frontend-guidelines / backend-guidelines 只保留总边界；具体链路和模块规则必须落到 `.trae/modules/`。
- [ ] 文档重写后必须能从 module map 追踪到每条正式读链路、写链路、cache invalidation 链路和 UI ViewModel 链路。

### Phase 2.9: 顶层文档与完成状态收口

- [ ] 修整 `.trae/frontend-guidelines.md`，只保留 Phase 2 新前端架构的长期总边界、层级职责、入口规则和验证目标。
- [ ] 修整 `.trae/backend-guidelines.md`，只保留 Phase 2 新后端架构的长期总边界、层级职责、command/service/domain/io/parser 规则和验证目标。
- [ ] 修整 `.trae/overview.md`，只保留当前项目状态、正式架构方向和必要总览，不写模块细节。
- [ ] 清理 frontend/backend guide 与 overview 中应归属模块文档的细节；模块文档已覆盖的内容不得重复堆在顶层文档。
- [ ] 根据 Phase 2.1 到 2.8 的实际完成状态勾选 todo；只勾已实现、已验证、文档已同步的条目。
- [ ] 跑文档格式、编码检查和前后端必要验证，确保完成状态不是仅靠文字判断。
...
## Phase 13: 禁止项：社区库数据文件集成（MagicLib / GraphicsLib / LazyLib）
