...
- [ ] 在总览之外的各标签栏界面中实现画布内自动吸附最近点并视作选中，吸附距离不限，并同步右侧编辑栏高亮。
- [ ] 在总览之外的各标签栏界面中保留画布外手动点击选择行为，避免只能依赖自动吸附。
- [ ] 明确武器、甲板、引擎、边界、范围各模式下“自动吸附选中”和“手动点击选中”的优先级与切换规则。
- [ ] 为舰船编辑器大修补手动验收清单，覆盖自动吸附、手动选择、右侧同步和模式切换。

## Phase 1.1: 完善舰船编辑器快捷键

- [ ] 支持 `P` 进入总览、`C` 进入范围、`B` 进入边界、`W` 进入武器、`L` 进入甲板、`E` 进入引擎。
- [ ] 支持 `T` 快速打开右侧标签栏并选中当前物件。
- [ ] 明确这些快捷键与输入框聚焦、右侧表单编辑、系统快捷键和现有撤销/重做的冲突处理规则。
- [ ] 在合适位置提供舰船编辑器快捷键提示，避免快捷键完全隐蔽。

## Phase 1.2: 支持 Shift / Ctrl / Alt 左键行为

- [ ] 范围：允许直接拖动舰船中心和护盾中心；`Shift+左键` 预览并重设 `collisionRadius`；`Ctrl+左键` 预览并重设 `shieldRadius`；角度或半径相关数值按 1 度或当前既定精度处理。
- [ ] 边界：`Shift+左键` 按鼠标位置在最末端插入节点；`Ctrl+左键` 按鼠标位置在最近边段插入节点。
- [ ] 武器：普通左键改为旋转槽位角度，角度 clamp 到 1 度；`Shift+左键` 按鼠标位置复制一个新武器槽位；`Ctrl+左键` 才执行拖动；`Alt+左键` 按鼠标位置修改武器允许射角，射角 clamp 到 1 度。
- [ ] 甲板：`Shift+左键` 按鼠标位置新建一个新甲板槽位。
- [ ] 引擎：普通左键改为旋转引擎角度，角度 clamp 到 1 度；`Shift+左键` 按鼠标位置复制一个新引擎；`Ctrl+左键` 才执行拖动引擎。
- [ ] 为这些组合操作补操作指南，并明确它们与右键平移、自动吸附选中和撤销链路的协同方式。

## Phase 1.3: 舰船/武器编辑器表单与贴图行为整理

- [ ] 舰船编辑器和武器编辑器中所有数字输入控件不再显示加号和减号。
- [ ] 修复当前折叠展开行为错误，确保各分组展开/收起状态稳定、间距正常、不会串状态。
- [ ] 舰船编辑器“贴图”分组移除“选择已有”能力。
- [ ] 舰船编辑器“贴图”分组新增“更新贴图宽高”按钮，把船体属性中的 `width` 和 `height` 同步为当前贴图尺寸。
- [ ] 为贴图宽高同步、折叠行为和数字输入控件补最小手动验收清单。
